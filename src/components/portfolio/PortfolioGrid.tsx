          ...
          {filteredProjects.map((project) => {
            return (
              <motion.article
                ...
                className="group relative flex flex-col overflow-hidden rounded-lg bg-card/70 border border-border/80 hover:border-foreground/30 hover:shadow-2xl transition-all duration-300"
              >
                <a
                ...
                >
                  ...
                  <div className="absolute top-3 left-3 flex items-center gap-1.5">
                    <span className="rounded-full bg-black/60 backdrop-blur-md px-2.5 py-1 text-[11px] font-medium text-white/90 border border-white/10 shadow-sm">
                      {project.role}
                    </span>
                  </div>

                  <div className="absolute top-3 right-3">
                    <span className="rounded-full bg-black/60 backdrop-blur-md px-2 py-1 text-[11px] font-mono text-white/80 border border-white/10">
                      {project.year}
                    </span>
                  </div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:scale-100 scale-90">
                    <span className="flex size-13 items-center justify-center rounded-full bg-white/95 text-black shadow-xl backdrop-blur-sm transition-transform duration-300 group-hover:scale-110">
                      <Play className="ml-0.5 size-5 fill-black" aria-hidden="true" />
                    </span>
                  </div>
                </a>
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                        {project.client || "Commercial Project"}
                      </p>
                      {project.tools?.[0] && (
                        <span className="text-[10px] font-mono text-muted-foreground/80 bg-secondary px-2 py-0.5 rounded border border-border/50">
                          {project.tools[0]}
                        </span>
                      )}
                    </div>
                    <h3 className="mt-2 line-clamp-1 text-base font-semibold tracking-tight text-foreground group-hover:text-primary transition-colors">
                      {project.title}
                    </h3>
                  </div>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-border/60 pt-3">
                    <div className="flex flex-wrap gap-1.5">
                      {project.tags.slice(0, 2).map((tag) => (
                        <span key={tag} className="text-[10px] text-muted-foreground/90">
                          #{tag}
                        </span>
                      ))}
                    </div>
