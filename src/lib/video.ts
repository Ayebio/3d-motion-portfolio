export function getBilibiliEmbedUrl(url?: string) {
  ...
  try {
    ...
    if (/^BV[0-9A-Za-z]+$/i.test(trimmed)) {
      return `https://player.bilibili.com/player.html?isOutside=true&bvid=${trimmed}&page=1&high_quality=1&danmaku=0`
    }
    ...
    if (host === "bilibili.com" || host === "m.bilibili.com") {
      ...
      if (match && match[1]) {
        return `https://player.bilibili.com/player.html?isOutside=true&bvid=${match[1]}&page=1&high_quality=1&danmaku=0`
      }
    }
